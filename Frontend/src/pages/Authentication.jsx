...
import { AuthContext } from '../contents/authentication.jsx';
  ...
  const handleAuth = async () => {
    try {
      if(formState === 0) {
        ...
        console.log(result);
        setMessage(result?.message || result?.data?.message || 'Login successful');
        setOpen(true);
      }
      if(formState === 1){
        ...
        console.log(result); 
        setUsername(null)
        setMessage(typeof result === 'string' ? result : result?.message || 'Registration successful');
        setOpen(true);
        setError("");
        setFormState(0);
        setPassword("");
